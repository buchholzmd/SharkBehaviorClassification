...
def main(config, **args):
    ...
    X_train, Y_train = sample_sequences(df_dict['train'], features, num_samples=num_train, seq_len=seq_len, dims=dims)
    X_train, Y_train = shuffle(X_train, Y_train, random_state=33)
    print()

    print('Sampling validation sequences')
    X_val, Y_val = sample_sequences(df_dict['val'], features, num_samples=num_val,  seq_len=seq_len, dims=dims)
    X_val, Y_val = shuffle(X_val, Y_val, random_state=33)
    print()

    print('Sampling testing sequences')
    if split_type == 'experiment': 
        X_test, Y_test = sample_sequences(df_dict['test'], features, dims=dims, test=True)
    elif split_type == 'full':
        if test_split == 'orig':
            X_test, Y_test = sample_sequences(df_dict['test'], features, seq_len=seq_len, dims=dims, test=True)

        else:
            X_test, Y_test = sample_sequences(df_dict['test'], features, num_samples=num_test, seq_len=seq_len, dims=dims)
    ...
    split_path = os.path.join(split_path, split_type + '_split_' + str(seq_len))
    ...
    test_path = os.path.join(data_path, 'test/' + test_split)
